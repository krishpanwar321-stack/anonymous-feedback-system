...
async function login() {
  ...
  if (res.ok) {
    ...
    // REQUIRED for dashboard & billing
    localStorage.setItem("userId", data.userId);
    localStorage.setItem("email", email);
    
    // keep ONLY if you don’t want to break old code
...
if (formId) {
  ...
  async function signup() {
    ...
    if (res.ok) {
      // store email so billing works after first login
      localStorage.setItem("email", email);
      window.location.href = "/login.html";
    }else {
      document.getElementById("error").innerText = data.error;
...
// 🔒 Prevent mobile Enter key from auto-submitting forms
document.addEventListener("keydown", function (e) {
  if (e.key === "Enter") {
    const isLoginPage = document.getElementById("password") && document.getElementById("email");
    const isSignupPage = document.getElementById("confirm");
